...
<div class="content-wrapper">
	...
	if(isset($_POST['preview'])){ // Jika user menekan tombol Preview pada form 
		...
		foreach($sheet as $row){ 
			...
			$kelurahan = $row['N']; // Ambil data nama
			$kecamatan = $row['O']; // Ambil data nama
			
			
			// Cek jika semua data tidak diisi
			if(empty($NIK) && empty($nama) && empty($jenis_kel) && empty($tempat_lahir) && empty($tgl_lahir) && empty($kewarganegaraan) && empty($agama) && empty($status) && empty($pendidikan_ter) && empty($pekerjaan) && empty($alamat) && empty($RT) && empty($RW)&& empty($RW)&& empty($kelurahan)&& empty($kecamatan))
			...
			if($numrow > 1){
				...
				$kelurahan_td = ( ! empty($kelurahan))? "" : " style='background: #E07171;'";
				$kecamatan_td = ( ! empty($kecamatan))? "" : " style='background: #E07171;'";
				
				// Jika salah satu data ada yang kosong
				if(empty($NIK) or empty($nama) or empty($jenis_kel) or empty($tempat_lahir) or empty($tgl_lahir) or empty($kewarganegaraan) or empty($agama) or empty($status) or empty($pendidikan_ter) or empty($pekerjaan) or empty($alamat) or empty($RT) or empty($RW) or empty($kelurahan) or empty($kecamatan) ){
