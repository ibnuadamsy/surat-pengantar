...
if (!defined('BASEPATH'))
  ...
  public function get_validasiKEL($status_rw = '2', $status_kel = '1'){
      $this->db->select('pengantar.*, penduduk.NIK AS NIK, penduduk.nama');
      $this->db->join('penduduk', 'pengantar.NIK = penduduk.NIK');
      $this->db->where('status_rw', $status_rw);
      $this->db->where('status_kel', $status_kel);
      $data = $this->db->get('pengantar');
      return $data->result();
  }

  public function get_valiKEL($status_kel = '2'){
    $this->db->select('pengantar.*, penduduk.NIK AS NIK, penduduk.nama');
    $this->db->join('penduduk', 'pengantar.NIK = penduduk.NIK');
    $this->db->where('status_kel', $status_kel);
    $data = $this->db->get('pengantar');
    ...
    public function get_cetak($no){
      $this->db->select('pengantar.*, penduduk.NIK AS NIK, penduduk.nama, penduduk.jenis_kel, penduduk.tempat_lahir, penduduk.tgl_lahir, penduduk.kewarganegaraan, penduduk.agama, penduduk.status, penduduk.pendidikan_ter, penduduk.alamat, penduduk.pekerjaan,penduduk.RT,penduduk.RW,penduduk.kelurahan,penduduk.kecamatan');
      $this->db->join('penduduk', 'pengantar.NIK = penduduk.NIK');
